...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="FOSIGA/ADRES: abrir consulta EPS y escribir número de documento."
    )
    parser.add_argument(
        "--numero",
        default="1022434547",
        help="Número de documento a diligenciar en el formulario.",
    )
    parser.add_argument(
        "-o",
        "--output-dir",
    ...
    parser.add_argument(
        "--registro-csv",
        default=None,
        help="Ruta del CSV de auditoría (por defecto: fosiga/fosiga_consultas.csv).",
    )
    parser.add_argument(
        "--headless",
        action="store_true",
        help="Ejecutar sin interfaz gráfica (no recomendado por validación/captcha del portal).",
    )
    parser.add_argument(
        "--keep-open-at-end",
        action="store_true",
        help="Mantener navegador abierto al finalizar (por defecto se cierra para finalizar rápido).",
    )
