...
def _cerrar_modal_inicial(page) -> None:
    ...
    try:
        ...
        logger.info("No apareció aviso inicial para cerrar.")


def _clic_boton_buscar(page, input_busqueda) -> None:
    # Se busca el boton exacto dentro del mismo formulario del input #search.
    ...
    # Fallback controlado: click forzado sobre el mismo boton exacto.
    try:
...
def _esperar_estado_consulta(page) -> str:
    ...
    while datetime.now().timestamp() - inicio < limite_s:
        try:
            ...
            # Reintenta durante la ventana de espera; pueden ocurrir recargas parciales.
            continue
...
def _abrir_rues(page) -> None:
    ...
    for url in URLS_RUES_FALLBACK:
        try:
            ...
            ultimo_error = exc
            # En algunos entornos RUES responde con codigo HTTP que Chromium marca
            # como fallo; intentamos con carga minima para continuar.
